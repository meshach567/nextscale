...
describe('Services Page', () => {
  it('renders the main heading', () => {
    render(<ServicesPage />);
    const heading = screen.getByRole('heading', { name: /our services/i, level: 1 });
    expect(heading).toBeInTheDocument();
  });

  it('renders the subheading text', () => {
    render(<ServicesPage />);
    expect(screen.getByText(/tailored solutions for billion-dollar industries in nigeria/i)).toBeInTheDocument();
  });
  ...
  it('renders the CTA strip with booking text', () => {
    render(<ServicesPage />);
    expect(screen.getByText(/not sure what service fits\? book a free call/i)).toBeInTheDocument();
  });

  it('renders the book call button', () => {
    render(<ServicesPage />);
    const button = screen.getByRole('button', { name: /book a free consultation call/i });
